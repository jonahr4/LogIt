/**
 * Log It — NCAA Football Backfill (standalone runner)
 * Run locally: npx tsx api/scripts/backfill-ncaaf.ts [seasons]
 * Example: npx tsx api/scripts/backfill-ncaaf.ts 2023,2024
 *
...
function getSeasonDateRange(startYear: number): { start: Date; end: Date } {
  return {
    start: new Date(startYear, 7, 15),
    end: new Date(startYear + 1, 0, 15),
  };
}
...
async function main() {
  ...
  console.log(`\n🏈 NCAA Football Backfill — Seasons: ${seasons.join(', ')}\n`);
