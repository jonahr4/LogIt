/**
 * Log It — NBA Sync (standalone runner)
 * Run locally: npx tsx api/scripts/sync-nba.ts
 *
 * Fetches NBA games for today ±7 days from ESPN and upserts into Supabase.
 * Same logic as api/cron/sync-nba.ts but runs as a CLI script.
 */
...
async function main() {
  ...
  if (allGames.length === 0) {
    console.log(`  ⚠ No NBA games found for date range`);
    return;
  }
