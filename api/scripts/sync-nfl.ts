/**
 * Log It — NFL Sync (standalone runner)
 * Run locally: npx tsx api/scripts/sync-nfl.ts
 *
 * Fetches NFL games for today ±7 days from ESPN and upserts into Supabase.
 * Same logic as api/cron/sync-nfl.ts but runs as a CLI script.
 */
...
async function main() {
  ...
  if (allGames.length === 0) {
    console.log(`  ⚠ No NFL games found for date range`);
    return;
  }
