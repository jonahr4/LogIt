/**
 * Log It — GET /api/cron/backfill-ncaaf
 * Backfill historical NCAA Football seasons from ESPN.
 *
 * Usage: GET /api/cron/backfill-ncaaf?seasons=2023,2024
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    for (const year of seasons) {
      ...
      console.log(`\n🏈 ── NCAA Football Backfill: Season ${seasonLabel} ──────────────────`);
      ...
      const seasonStart = new Date(year, 7, 15);
      const seasonEnd = new Date(year + 1, 0, 15);
    ...
    console.log(`\n🏈 ── NCAA Football Backfill Complete ──────────────────────`);
    console.log(`  Total: ${total} games inserted across ${seasons.length} season(s)\n`);

    return res.status(200).json({
      message: `Backfilled ${total} NCAA Football games across ${seasons.length} season(s)`,
      results,
    });
  } catch (error: any) {
    console.error('❌ NCAA Football backfill error:', error);
