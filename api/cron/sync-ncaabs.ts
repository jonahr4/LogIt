/**
 * Log It — GET /api/cron/sync-ncaabs
 * Vercel Cron Job: Syncs NCAA Baseball games from ESPN into Supabase
 *
...
function deriveNCAABSSeason(eventDate: string): string {
...
const CONFIG: SportConfig = {
  ...
  venueType: 'stadium',
  deriveSeason: deriveNCAABSSeason,
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (allGames.length === 0) {
      console.log(`  ⚠ No NCAA Baseball games found for date range`);
      return res.status(200).json({ message: 'No NCAA Baseball games found for date range', synced: 0 });
    }
    ...
    console.error('❌ NCAA Baseball sync error:', error);
    return res.status(500).json({
      error: { code: 'SERVER_ERROR', message: error.message || 'NCAA Baseball sync failed', status: 500 },
    });
  }
}
