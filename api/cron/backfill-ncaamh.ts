/**
 * Log It — GET /api/cron/backfill-ncaamh
 * Backfill historical NCAA Hockey seasons from ESPN.
 *
 * Usage: GET /api/cron/backfill-ncaamh?seasons=2023,2024
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    for (const year of seasons) {
      ...
      console.log(`\n🏒 ── NCAA Hockey Backfill: Season ${seasonLabel} ──────────────────`);
      ...
      const seasonEnd = new Date(year + 1, 3, 15);
    ...
    console.log(`\n🏒 ── NCAA Hockey Backfill Complete ──────────────────────`);
    console.log(`  Total: ${total} games inserted across ${seasons.length} season(s)\n`);

    return res.status(200).json({
      message: `Backfilled ${total} NCAA Hockey games across ${seasons.length} season(s)`,
      results,
    });
  } catch (error: any) {
    console.error('❌ NCAA Hockey backfill error:', error);
