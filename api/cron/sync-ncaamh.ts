/**
 * Log It — GET /api/cron/sync-ncaamh
 * Vercel Cron Job: Syncs NCAA Hockey games from ESPN into Supabase
 *
...
function deriveNCAAMHSeason(eventDate: string): string {
...
const CONFIG: SportConfig = {
  ...
  venueType: 'arena',
  deriveSeason: deriveNCAAMHSeason,
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (allGames.length === 0) {
      console.log(`  ⚠ No NCAA Hockey games found for date range`);
      return res.status(200).json({ message: 'No NCAA Hockey games found for date range', synced: 0 });
    }
    ...
    console.error('❌ NCAA Hockey sync error:', error);
    return res.status(500).json({
      error: { code: 'SERVER_ERROR', message: error.message || 'NCAA Hockey sync failed', status: 500 },
    });
  }
}
