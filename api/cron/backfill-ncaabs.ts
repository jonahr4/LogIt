/**
 * Log It — GET /api/cron/backfill-ncaabs
 * Backfill historical NCAA Baseball seasons from ESPN.
 *
 * Usage: GET /api/cron/backfill-ncaabs?seasons=2024,2025
 *   - seasons: comma-separated years
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    for (const year of seasons) {
      ...
      console.log(`\n⚾ ── NCAA Baseball Backfill: Season ${seasonLabel} ──────────────────`);
      ...
      const seasonStart = new Date(year, 1, 15);
      const seasonEnd = new Date(year, 6, 1);
    ...
    console.log(`\n⚾ ── NCAA Baseball Backfill Complete ──────────────────────`);
    console.log(`  Total: ${total} games inserted across ${seasons.length} season(s)\n`);

    return res.status(200).json({
      message: `Backfilled ${total} NCAA Baseball games across ${seasons.length} season(s)`,
      results,
    });
  } catch (error: any) {
    console.error('❌ NCAA Baseball backfill error:', error);
