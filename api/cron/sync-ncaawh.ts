/**
 * Log It — GET /api/cron/sync-ncaawh
 * Vercel Cron Job: Syncs WNCAA Hockey games from ESPN into Supabase
 *
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (allGames.length === 0) {
      ...
      return res.status(200).json({ message: 'No WNCAA Hockey games found for date range', synced: 0 });
    }
    ...
    return res.status(200).json({
      message: `Synced ${synced} WNCAA Hockey games`,
      synced,
      updated,
      skipped,
    ...
    console.error('❌ WNCAA Hockey sync error:', error);
    return res.status(500).json({
      error: { code: 'SERVER_ERROR', message: error.message || 'WNCAA Hockey sync failed', status: 500 },
    });
  }
}
