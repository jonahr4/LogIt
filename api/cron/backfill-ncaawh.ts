/**
 * Log It — GET /api/cron/backfill-ncaawh
 * Backfill historical WNCAA Hockey seasons from ESPN.
 *
 * Usage: GET /api/cron/backfill-ncaawh?seasons=2023,2024
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    for (const year of seasons) {
      ...
      console.log(`\n🏒 ── WNCAA Hockey Backfill: Season ${seasonLabel} ──────────────────`);
    ...
    console.log(`\n🏒 ── WNCAA Hockey Backfill Complete ──────────────────────`);
    console.log(`  Total: ${total} games inserted across ${seasons.length} season(s)\n`);

    return res.status(200).json({
      message: `Backfilled ${total} WNCAA Hockey games across ${seasons.length} season(s)`,
      results,
    });
  } catch (error: any) {
    console.error('❌ WNCAA Hockey backfill error:', error);
