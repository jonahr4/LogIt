/**
 * Log It — GET /api/cron/sync-ncaaf
 * Vercel Cron Job: Syncs NCAA Football games from ESPN into Supabase
 *
...
function deriveNCAAFSeason(eventDate: string): string {
...
const CONFIG: SportConfig = {
  ...
  venueType: 'stadium',
  deriveSeason: deriveNCAAFSeason,
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (allGames.length === 0) {
      console.log(`  ⚠ No NCAA Football games found for date range`);
      return res.status(200).json({ message: 'No NCAA Football games found for date range', synced: 0 });
    }
    ...
    console.error('❌ NCAA Football sync error:', error);
    return res.status(500).json({
      error: { code: 'SERVER_ERROR', message: error.message || 'NCAA Football sync failed', status: 500 },
    });
  }
}
