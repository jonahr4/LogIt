/**
 * Log It — GET /api/cron/backfill-ncaam
 * Backfill historical NCAA Basketball seasons from ESPN.
 *
 * Usage: GET /api/cron/backfill-ncaam?seasons=2023,2024
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    for (const year of seasons) {
      ...
      console.log(`\n🏀 ── NCAA Basketball Backfill: Season ${seasonLabel} ──────────────────`);
    ...
    console.log(`\n🏀 ── NCAA Basketball Backfill Complete ──────────────────────`);
    console.log(`  Total: ${total} games inserted across ${seasons.length} season(s)\n`);

    return res.status(200).json({
      message: `Backfilled ${total} NCAA Basketball games across ${seasons.length} season(s)`,
      results,
    });
  } catch (error: any) {
    console.error('❌ NCAA Basketball backfill error:', error);
