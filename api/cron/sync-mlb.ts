/**
 * Log It — GET /api/cron/sync-mlb
 * Vercel Cron Job: Syncs MLB games from ESPN into Supabase
 * Schedule: daily at 6:15 AM UTC (configured in vercel.json)
 *
...
/** MLB uses single-year seasons based on the calendar year. */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const allGames = await fetchESPNScoreboard(MLB_CONFIG.espnPath);
    console.log(`  📡 ESPN returned ${allGames.length} games\n`);

    if (allGames.length === 0) {
      console.log(`  ⚠ No MLB games found for date range`);
      return res.status(200).json({ message: 'No MLB games found for date range', synced: 0 });
    }
    ...
    return res.status(200).json({
      message: `Synced ${synced} MLB games`,
      synced,
      updated,
      skipped,
    ...
    return res.status(500).json({
      error: { code: 'SERVER_ERROR', message: error.message || 'MLB sync failed', status: 500 },
    });
  }
}
