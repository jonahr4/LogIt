/**
 * Log It — GET /api/cron/sync-wnba
 * Vercel Cron Job: Syncs WNBA games from ESPN into Supabase
 * Schedule: daily at 6:20 AM UTC (configured in vercel.json)
 *
...
/** WNBA uses single-year seasons based on the calendar year. */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const allGames = await fetchESPNScoreboard(WNBA_CONFIG.espnPath);
    console.log(`  📡 ESPN returned ${allGames.length} games\n`);

    if (allGames.length === 0) {
      console.log(`  ⚠ No WNBA games found for date range`);
      return res.status(200).json({ message: 'No WNBA games found for date range', synced: 0 });
    }
    ...
    return res.status(200).json({
      message: `Synced ${synced} WNBA games`,
      synced,
      updated,
      skipped,
    ...
    console.error('❌ WNBA sync error:', error);
    return res.status(500).json({
      error: { code: 'SERVER_ERROR', message: error.message || 'WNBA sync failed', status: 500 },
    });
  }
}
