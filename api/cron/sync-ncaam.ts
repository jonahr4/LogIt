/**
 * Log It — GET /api/cron/sync-ncaam
 * Vercel Cron Job: Syncs NCAA Basketball games from ESPN into Supabase
 *
...
function deriveNCAAMSeason(eventDate: string): string {
...
const CONFIG: SportConfig = {
  ...
  venueType: 'arena',
  deriveSeason: deriveNCAAMSeason,
};
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (allGames.length === 0) {
      console.log(`  ⚠ No NCAA Basketball games found for date range`);
      return res.status(200).json({ message: 'No NCAA Basketball games found for date range', synced: 0 });
    }
    ...
    console.error('❌ NCAA Basketball sync error:', error);
    return res.status(500).json({
      error: { code: 'SERVER_ERROR', message: error.message || 'NCAA Basketball sync failed', status: 500 },
    });
  }
}
