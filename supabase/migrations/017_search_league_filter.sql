-- 017: Add league_filter parameter to search_events
-- Moves league filtering from post-filter to SQL WHERE clause
-- so it works correctly with LIMIT/OFFSET pagination.
...
BEGIN
  ...
  FROM (
    ...
    AND (league_filter IS NULL OR se.league = league_filter)
  ) sub
